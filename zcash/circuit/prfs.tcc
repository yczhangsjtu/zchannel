...
public:
    ...
    ) : gadget<FieldT>(pb,annotation), result(result) {
...
public:
    PRF_addr_a_pk_gadget(
        ...
        std::shared_ptr<digest_variable<FieldT>> result,
				const std::string& annotation
    ) : PRF_gadget<FieldT>(pb, ZERO, 1, 1, 0, 0, a_sk, gen256zeroes(ZERO), result, annotation) {}
};
...
public:
    PRF_nf_gadget(
        ...
        std::shared_ptr<digest_variable<FieldT>> result,
				const std::string& annotation
    ) : PRF_gadget<FieldT>(pb, ZERO, 1, 1, 1, 0, a_sk, rho, result, annotation) {}
};
...
public:
    PRF_pk_gadget(
        ...
        std::shared_ptr<digest_variable<FieldT>> result,
				const std::string& annotation
    ) : PRF_gadget<FieldT>(pb, ZERO, 0, nonce, 0, 0, a_sk, h_sig, result, annotation) {}
};
...
public:
    PRF_rho_gadget(
        ...
        std::shared_ptr<digest_variable<FieldT>> result,
				const std::string& annotation
    ) : PRF_gadget<FieldT>(pb, ZERO, 0, nonce, 1, 0, phi, h_sig, result, annotation) {}
};
